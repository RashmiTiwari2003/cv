    ...
    <div id='projects' className='flex flex-col justify-center items-center py-14 w-full'>
      ...
      <div className='flex justify-center items-center gap-2 md:gap-8 mt-4 mb-6'>
        {Category.map((cat, ind) => (
          <button key={ind} onClick={() => setCategory(cat)} className={`${cat===category?'bg-white border-b-4 border-blue-800 mb-0 font-bold text-black':'text-white bg-blue-800 mb-4 text-sm' } flex justify-center items-center hover:bg-white hover:mb-0 px-4 py-2 hover:border-b-4 hover:border-blue-800 rounded-lg md:w-28 hover:font-bold sm:text-base hover:text-black capitalize`}>{cat}</button>
          // <button key={ind} onClick={() => setCategory(cat)} className={`${cat===category?'bg-white border-b-4 border-blue-800 mb-0 font-bold text-black':'' } flex justify-center items-center bg-blue-800 hover:bg-white mb-4 hover:mb-0 px-4 py-2 hover:border-b-4 hover:border-blue-800 rounded-lg md:w-28 hover:font-bold text-sm text-white sm:text-base hover:text-black capitalize`}>{cat}</button>
        ))}
      </div>
